...
4. **Real deep-link checkout** (`src/services/bookingLink.ts`) — builds a URL into eurotrain.net's real search-results page using the user's actual origin/destination/date(s)/passengers (round trip in a single link), opened via `Linking.openURL`. Station ids are eurotrain.net's own `bookingSlug`s, captured by hand walking the live site. No payment form, no card data, no account ever touches this app — the same metasearch/click-out pattern as Skyscanner or Google Flights. (Redirect target was switched from eurostar.com to eurotrain.net mid-build; the same research-first verification was re-run for the new site.)
...
6. **Real, live-refreshed departures** (`scripts/fetch-live-schedules.mjs`, `src/services/liveScheduleService.ts`) — Eurostar/Thalys's official open GTFS + GTFS-RT feed (free, keyless, France's Licence Ouverte 2.0). A scheduled GitHub Actions workflow (`refresh-schedules.yml`) fetches + parses it and publishes `live-schedules.json` to a `data` branch; the app reads it with a timeout and in-memory cache. (Originally scoped as a Cloudflare Worker; pivoted to GitHub Actions after finding Cloudflare's free-tier 10ms-CPU-per-invocation cap couldn't handle GTFS parsing.) Live departures get a "Canlı" badge; uncovered routes fall back to the synthetic generator, clearly labelled either way. One-time setup: point `LIVE_SCHEDULE_REPO` at your own repo (§2).
...
- Real Rail Europe pricing/booking API behind the existing `services/` seam — genuinely live *prices*, not just times
...
- A stronger connectivity check — active reachability probe, not just `navigator.onLine`/OS state
