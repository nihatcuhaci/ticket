...
4. **Real deep-link checkout** (`src/services/bookingLink.ts`): builds a URL into eurotrain.net's real search-results page using the user's actual origin/destination/date(s)/passengers (round trip in a single link), opened via `Linking.openURL`. Station ids are eurotrain.net's own `bookingSlug`s, captured by hand walking the live site. No payment form, no card data, no account ever touches this app. This is the same metasearch/click-out pattern as Skyscanner or Google Flights. (Redirect target was switched from eurostar.com to eurotrain.net mid-build; the same research-first verification was re-run for the new site.)
